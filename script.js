...
window.toggleSidebarJogador = function(numSlot) {
    ...
    if (sidebar.classList.contains('sidebar-fechada')) {
        ...
        // ══════════════════════════════════════════════════════════
        //  SISTEMA DE BARRAS MÁGICAS — CANVAS 2D AAA
        //  60 FPS | GPU-friendly | Loop perfeito | Zero stutter
        // ══════════════════════════════════════════════════════════
        (function initBarrasMagicas() {
            // Registra todos os canvas que precisam ser criados e animados
            const barrasRegistradas = new Map();
            // { canvas, ctx, tipo, getWidth, getAlert, t }

            // ─── Utilitários ───────────────────────────────────────
            function lerp(a, b, t) { return a + (b - a) * t; }
            function clamp(v, lo, hi) { return Math.max(lo, Math.min(hi, v)); }

            // Noise pseudo-aleatório determinístico (sem Math.random no loop)
            function noise(x) {
                const X = Math.floor(x) & 255;
                x -= Math.floor(x);
                const fade = x * x * x * (x * (x * 6 - 15) + 10);
                const a = (X * 1664525 + 1013904223) & 0xffffffff;
                const b2 = ((X + 1) * 1664525 + 1013904223) & 0xffffffff;
                const ga = (a >>> 16) / 65535 * 2 - 1;
                const gb = (b2 >>> 16) / 65535 * 2 - 1;
                return lerp(ga * x, gb * (x - 1), fade);
            }

            // ─── Criação do Canvas sobre .bar-fill ─────────────────
            function criarCanvasNaBarra(barFill, tipo) {
                if (barFill.dataset.canvasInjetado) return;
                barFill.dataset.canvasInjetado = '1';

                const cv = document.createElement('canvas');
                cv.className = 'bar-canvas';
                barFill.appendChild(cv);

                const ctx = cv.getContext('2d', { alpha: true });

                const entry = {
                    cv, ctx, tipo,
                    t: Math.random() * 1000,   // fase inicial aleatória
                    particles: [],
                    runes: tipo === 'mana' ? criarRunas() : null
                };

                // Pré-cria partículas
                const qtd = tipo === 'hp' ? 14 : 18;
                for (let i = 0; i < qtd; i++) {
                    entry.particles.push(criarParticula(tipo));
                }

                barrasRegistradas.set(cv, entry);
            }
            ...
            function atualizarParticula(p, tipo) {
                p.x += p.vx;
                p.y += p.vy;
                p.life += 0.003;
                // Deriva suave horizontal
                p.vx += (Math.random() - 0.5) * 0.00004;
                if (p.life >= p.maxLife || p.x < 0 || p.x > 1 || p.y < -0.1) {
                    Object.assign(p, criarParticula(tipo));
                    p.x = Math.random();
            ...
            // ─── Runas (apenas Mana) ───────────────────────────────
            const RUNE_CHARS = ['ᚠ','ᚢ','ᚦ','ᚨ','ᚱ','ᚲ','ᚷ','ᚹ','ᚺ','ᚾ'];
            function criarRunas() {
            ...
            // ─── Renderização HP ───────────────────────────────────
            function renderHP(cv, ctx, entry, W, H, fill, alert, t) {
                ctx.clearRect(0, 0, W, H);
                if (fill <= 0) return;

                const fw = W * fill;   // largura preenchida
                const cx = fw / 2;
                const cy = H / 2;

                // ── Núcleo base — gradiente radial vivo ──
                const baseAlpha = alert
                    ? 0.55 + 0.25 * Math.sin(t * 0.12)
                    : 0.45 + 0.15 * Math.sin(t * 0.04);

                const core = ctx.createLinearGradient(0, 0, fw, 0);
                if (alert) {
                    core.addColorStop(0,    `rgba(80,20,0,${baseAlpha})`);
                    core.addColorStop(0.25, `rgba(200,80,0,${baseAlpha + 0.1})`);
                    core.addColorStop(0.5,  `rgba(255,180,0,${baseAlpha + 0.2})`);
                    core.addColorStop(0.75, `rgba(200,80,0,${baseAlpha + 0.1})`);
                    core.addColorStop(1,    `rgba(80,20,0,${baseAlpha})`);
                } else {
                    core.addColorStop(0,    `rgba(8,60,25,${baseAlpha})`);
                    core.addColorStop(0.2,  `rgba(15,120,50,${baseAlpha + 0.1})`);
                    core.addColorStop(0.5,  `rgba(30,220,90,${baseAlpha + 0.25})`);
                    core.addColorStop(0.8,  `rgba(15,120,50,${baseAlpha + 0.1})`);
                    core.addColorStop(1,    `rgba(8,60,25,${baseAlpha})`);
                }
                ctx.save();
                ctx.beginPath();
                roundRect(ctx, 0, 0, fw, H, H / 2);
                ctx.clip();
                ctx.fillStyle = core;
                ctx.fillRect(0, 0, fw, H);

                // ── Vórtice / ondas internas ──
                const waves = alert ? 3 : 4;
                for (let i = 0; i < waves; i++) {
                    const phase  = t * (alert ? 0.10 : 0.05) + (i / waves) * Math.PI * 2;
                    const amp    = H * (alert ? 0.32 : 0.22);
                    const yBase  = cy + amp * Math.sin(phase);
                    const alpha2 = alert
                        ? 0.12 + 0.08 * Math.sin(t * 0.09 + i)
                        : 0.08 + 0.05 * Math.sin(t * 0.03 + i);

                    const wg = ctx.createLinearGradient(0, 0, fw, 0);
                    if (alert) {
                        wg.addColorStop(0,   'rgba(255,100,0,0)');
                        wg.addColorStop(0.5, `rgba(255,200,50,${alpha2 * 2})`);
                        wg.addColorStop(1,   'rgba(255,100,0,0)');
                    } else {
                        wg.addColorStop(0,   'rgba(20,200,80,0)');
                        wg.addColorStop(0.5, `rgba(100,255,130,${alpha2 * 2})`);
                        wg.addColorStop(1,   'rgba(20,200,80,0)');
                    }
                    ...
                    for (let x = 0; x <= fw; x += 3) {
                        const ny = yBase
                            + amp * 0.4 * Math.sin((x / fw) * Math.PI * 6 + phase)
                            + amp * 0.2 * noise(x / fw * 3 + t * 0.02);
                        ctx.lineTo(x, ny);
                    }
                    ctx.lineTo(fw, H); ctx.lineTo(0, H); ctx.closePath();
                    ctx.fillStyle = wg;
                    ctx.fill();
                }
                ...
                // ── Reflexo superior ──
                const refH  = H * 0.35;
                const refGr = ctx.createLinearGradient(0, 0, 0, refH);
                refGr.addColorStop(0, alert
                    ? 'rgba(255,220,100,0.25)'
                    : 'rgba(180,255,200,0.22)');
                refGr.addColorStop(1, 'rgba(0,0,0,0)');
                ctx.fillStyle = refGr;
                ctx.fillRect(0, 0, fw, refH);

                // ── Partículas etéreas ──
                entry.particles.forEach(p => {
                    atualizarParticula(p, 'hp');
                    const px = p.x * fw;
                    const py = p.y * H;
                    const alpha3 = clamp(
                        Math.sin((p.life / p.maxLife) * Math.PI) * (alert ? 0.9 : 0.7),
                        0, 1
                    );
                    const col = alert
                        ? `rgba(255,${180 + Math.floor(p.life * 40)},0,${alpha3})`
                        : `rgba(${100 + Math.floor(p.life * 80)},255,${140 + Math.floor(p.life * 50)},${alpha3})`;
                    ctx.beginPath();
                    ctx.arc(px, py, p.size, 0, Math.PI * 2);
                    ctx.fillStyle = col;
                    ctx.fill();

                    // Partícula dourada em HP cheio
                    if (!alert && fill > 0.95 && Math.random() < 0.002) {
                        ctx.beginPath();
                        ctx.arc(px, py, p.size + 1, 0, Math.PI * 2);
                        ctx.fillStyle = `rgba(255,215,0,${alpha3 * 0.6})`;
                        ctx.fill();
                    }
                });

                // ── Rachadura de alerta ──
                if (alert && Math.random() < 0.015) {
                    const rx = Math.random() * fw;
                    const rl = 4 + Math.random() * 10;
                    ctx.beginPath();
                    ctx.moveTo(rx, Math.random() * H);
                    ctx.lineTo(rx + (Math.random() - 0.5) * 6, Math.random() * H);
                    ctx.strokeStyle = `rgba(255,200,50,${0.3 + Math.random() * 0.4})`;
                    ctx.lineWidth = 0.8;
                    ctx.stroke();
                }
                ...
                // ── Aura externa (fora do clip) ──
                const auraA = alert
                    ? 0.55 + 0.35 * Math.sin(t * 0.12)
                    : 0.35 + 0.2  * Math.sin(t * 0.04);
                const auraCol = alert
                    ? `rgba(255,120,0,${auraA})`
                    : `rgba(46,204,113,${auraA})`;
                cv.style.filter = `drop-shadow(0 0 ${alert ? 6 + 4 * Math.sin(t * 0.12) : 5 + 3 * Math.sin(t * 0.04)}px ${auraCol})`;
            }

            // ─── Renderização Mana ─────────────────────────────────
            function renderMana(cv, ctx, entry, W, H, fill, full, t) {
                ctx.clearRect(0, 0, W, H);
                if (fill <= 0) return;
                ...
                // ── Fundo cósmico ──
                const baseA = 0.45 + 0.15 * Math.sin(t * 0.035);
                const bg = ctx.createLinearGradient(0, 0, fw, 0);
                bg.addColorStop(0,    `rgba(5,15,50,${baseA})`);
                bg.addColorStop(0.25, `rgba(10,40,100,${baseA + 0.12})`);
                bg.addColorStop(0.5,  `rgba(20,80,180,${baseA + 0.25})`);
                bg.addColorStop(0.75, `rgba(10,40,100,${baseA + 0.12})`);
                bg.addColorStop(1,    `rgba(5,15,50,${baseA})`);
                ...
                // ── Correntes de plasma central ──
                for (let s = 0; s < 3; s++) {
                    const phase = t * 0.055 + s * (Math.PI * 2 / 3);
                    const amp   = H * 0.18;
                    const alpha4 = 0.15 + 0.08 * Math.sin(t * 0.04 + s);
                    const sg = ctx.createLinearGradient(0, 0, fw, 0);
                    sg.addColorStop(0,   'rgba(0,150,255,0)');
                    sg.addColorStop(0.4, `rgba(50,180,255,${alpha4 * 1.5})`);
                    sg.addColorStop(0.5, `rgba(150,230,255,${alpha4 * 2})`);
                    sg.addColorStop(0.6, `rgba(50,180,255,${alpha4 * 1.5})`);
                    sg.addColorStop(1,   'rgba(0,150,255,0)');

                    ctx.beginPath();
                    const cy2 = H / 2 + amp * Math.sin(phase);
                    ctx.moveTo(0, cy2);
                    for (let x = 0; x <= fw; x += 3) {
                        ctx.lineTo(x,
                            H / 2
                            + amp * Math.sin((x / fw) * Math.PI * 5 + phase)
                            + amp * 0.3 * noise(x / fw * 4 + t * 0.025 + s));
                    }
                    ctx.lineTo(fw, H); ctx.lineTo(0, H); ctx.closePath();
                    ctx.fillStyle = sg;
                    ctx.fill();
                ...
                // ── Poeira estelar ──
                entry.particles.forEach(p => {
                    atualizarParticula(p, 'mana');
                    const px = p.x * fw;
                    const py = p.y * H;
                    const a5 = clamp(Math.sin((p.life / p.maxLife) * Math.PI) * 0.85, 0, 1);
                    const twinkle = 0.5 + 0.5 * Math.sin(t * 0.2 + p.x * 10);
                    ctx.beginPath();
                    ctx.arc(px, py, p.size * (0.5 + 0.5 * twinkle), 0, Math.PI * 2);
                    ctx.fillStyle = `rgba(${180 + Math.floor(twinkle * 75)},${220 + Math.floor(twinkle * 35)},255,${a5 * twinkle})`;
                    ctx.fill();

                    // Micro-raio de consumo (efeito ocasional)
                    if (full < 0.95 && Math.random() < 0.004) {
                        ctx.beginPath();
                        ctx.moveTo(px, py);
                        ctx.lineTo(px + (Math.random() - 0.5) * 8, py + (Math.random() - 0.5) * 8);
                        ctx.strokeStyle = `rgba(100,200,255,${0.4 + Math.random() * 0.4})`;
                        ctx.lineWidth = 0.5;
                        ctx.stroke();
                    }
                });
                ...
                // ── Runas surgindo ──
                if (full > 0.9 && entry.runes) {
                    ctx.font = `${H * 0.55}px serif`;
                    ctx.textBaseline = 'middle';
                    entry.runes.forEach(r => {
                        r.phase += r.speed;
                        if (r.phase > Math.PI * 2) {
                            r.char = RUNE_CHARS[Math.floor(Math.random() * RUNE_CHARS.length)];
                            r.x = Math.random() * 0.85 + 0.05;
                            r.phase = 0;
                        }
                        const ra = clamp(Math.sin(r.phase) * 0.5, 0, 1);
                        if (ra < 0.01) return;
                        ctx.fillStyle = `rgba(150,220,255,${ra * 0.6})`;
                        ctx.fillText(r.char, r.x * fw, H / 2);
                    });
                }

                // ── Reflexo superior ──
                const rfGr = ctx.createLinearGradient(0, 0, 0, H * 0.4);
                rfGr.addColorStop(0, `rgba(100,180,255,${0.20 + 0.08 * Math.sin(t * 0.035)})`);
                rfGr.addColorStop(1, 'rgba(0,0,0,0)');
                ctx.fillStyle = rfGr;
                ctx.fillRect(0, 0, fw, H * 0.4);

                ctx.restore();

                // ── Aura cósmica ──
                const auraIntens = 0.45 + 0.25 * Math.sin(t * 0.04);
                cv.style.filter = `drop-shadow(0 0 ${5 + 3 * Math.sin(t * 0.04)}px rgba(52,152,219,${auraIntens})) drop-shadow(0 0 ${2 + 1.5 * Math.sin(t * 0.07)}px rgba(150,230,255,${auraIntens * 0.6}))`;
            }

            // ─── roundRect helper (sem Path2D para compat. máxima) ─
            ...
            // ─── Detecta e injeta canvas em todos os .bar-fill ─────
            function escanearEInjetar() {
                document.querySelectorAll('.hp-fill:not([data-canvas-injetado])').forEach(el => {
                    criarCanvasNaBarra(el, 'hp');
                });
                document.querySelectorAll('.mana-fill:not([data-canvas-injetado])').forEach(el => {
                    criarCanvasNaBarra(el, 'mana');
                });
            }
            ...
            function loop(ts) {
                ...
                // Throttle: não passar de 60 FPS mas aceitar telas de 120hz
                const dt = ts - ultimoTs;
                if (dt < 14) return;   // ~70 FPS max — evita excesso em 120hz
                ultimoTs = ts;

                // Escaneia novos .bar-fill que possam ter sido injetados pelo Firebase
                escanearEInjetar();

                barrasRegistradas.forEach((entry) => {
                    const { cv, ctx, tipo } = entry;

                    // Verifica se ainda está no DOM
                    if (!document.body.contains(cv)) {
                        barrasRegistradas.delete(cv);
                        return;
                    }

                    const barFill = cv.parentElement;
                    if (!barFill) return;

                    const barBg = barFill.parentElement;
                    if (!barBg) return;

                    // Dimensões reais
                    const W = barBg.offsetWidth  || 200;
                    const H = barBg.offsetHeight || 28;
                    ...
                    // Fill ratio a partir da largura do barFill
                    const fillPct = parseFloat(barFill.style.width) || 0;
                    const fill    = clamp(fillPct / 100, 0, 1);
                    const alert   = fillPct <= 20 && fillPct > 0;
                    const full    = fillPct >= 95;

                    entry.t += 0.7;   // velocidade do tempo interno

                    if (tipo === 'hp') {
                        renderHP(cv, ctx, entry, W, H, fill, alert, entry.t);
                    } else {
                        renderMana(cv, ctx, entry, W, H, fill, full, entry.t);
                    }
                });
            }
            ...
            // ─── Observer: detecta novos slots abertos pelo Firebase ─
            const observer = new MutationObserver(() => {
                escanearEInjetar();
            });
            observer.observe(document.getElementById('slot-1'), { childList: true, subtree: true });
            observer.observe(document.getElementById('slot-2'), { childList: true, subtree: true });

            // Expõe para debug
            window._barrasCanvas = barrasRegistradas;
        })();
        ...
        window.abrirGrimorio = function(numSlot) { console.log('ABRINDO GRIMORIO', slotsDeVisao[numSlot].dados.grimorio);
            ...
            // RAF garante que display:flex seja computado antes de adicionar a classe
            // — evita o travamento causado por forçar reflow síncrono
